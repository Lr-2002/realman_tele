...
class ArmUnit(DeviceUnit):
    ...
    def move_to_cartesian_position(self, position: List[float], orientation: List[float]) -> bool:
        """
...
class SocketArmController(DeviceController):
    ...
    def validate_control_signal(self, control_signal: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
        ...
        if not isinstance(control_signal, dict):
            return False, f"Expected dictionary control signal, got {type(control_signal)}"
        ...
        if cmd == "joint_position":
            ...
            if "orientation" not in control_signal:
                return False, "Missing 'orientation' field for cartesian position command"
            ...
            if not isinstance(orientation, (list, tuple, np.ndarray)):
                return False, f"Expected list for orientation data, got {type(orientation)}"
...
class RealArmController(DeviceController):
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            from arms.robotic_arm_package.robotic_arm import Arm, RM75
    ...
    def validate_control_signal(self, control_signal: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
        ...
        if cmd == "joint_position":
            ...
            joint_data = control_signal.get("data")
            if not isinstance(joint_data, (list, tuple, np.ndarray)):
                return False, f"Expected array-like for joint data, got {type(joint_data)}"
                
            # 验证关节数量是否正确
            if isinstance(joint_data, (list, tuple)) and len(joint_data) != 7:
                return False, f"Realman arm requires 7 joint values, got {len(joint_data)}"
            elif isinstance(joint_data, np.ndarray) and joint_data.size != 7:
                return False, f"Realman arm requires 7 joint values, got {joint_data.size}"
            ...
            position = control_signal.get("position")
            if not isinstance(position, (list, tuple, np.ndarray)):
                return False, f"Expected array-like for position data, got {type(position)}"
                
            # 验证位置坐标是否是3维
            if (isinstance(position, (list, tuple)) and len(position) != 3) or \
               (isinstance(position, np.ndarray) and position.size != 3):
                return False, f"Position should have 3 values (x,y,z), got {len(position) if isinstance(position, (list, tuple)) else position.size}"
                
            # 验证姿态数据
            orientation = control_signal.get("orientation")
            if not isinstance(orientation, (list, tuple, np.ndarray)):
                return False, f"Expected array-like for orientation data, got {type(orientation)}"
                
            # 验证姿态是否是3维（欧拉角）
            if (isinstance(orientation, (list, tuple)) and len(orientation) != 3) or \
               (isinstance(orientation, np.ndarray) and orientation.size != 3):
                return False, f"Orientation should have 3 values (euler angles), got {len(orientation) if isinstance(orientation, (list, tuple)) else orientation.size}"
    ...
    def control(self, control_signal: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if cmd_type == "joint_position":
                ...
                joint_angles = np.array(control_signal.get("data", [0, 0, 0, 0, 0, 0, 0]))
    ...
    def get_state(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "position": pose[:3].tolist() if isinstance(pose, np.ndarray) else pose[:3],
                "orientation": pose[3:].tolist() if isinstance(pose, np.ndarray) else pose[3:],
            }
        except Exception as e:
